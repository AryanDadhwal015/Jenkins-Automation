pipeline {
  ...
  stages {
    ...
    // stage('Print Config') {
    //   steps {
    //     // read preview/dev paths from Jenkins credentials (same IDs used in deploy stages)
    //     withCredentials([
    ...
    //       sh '''
    //         # place previews under a "previews" web path so URL /previews/<branch>/ maps correctly
    ...
    //         # verify remote files and report if index.html exists to help diagnose 404
    ...
    //   when {
    //     expression {
    //       // compare the last path segment of BRANCH_NAME so "origin/dev", "refs/heads/dev" and "dev" match
    ...
    //       sh '''
    //         # ensure consistent previews subpath for docker deployments too
    ...
    //          ssh -i $SSH_KEY -o StrictHostKeyChecking=no $SSH_USER@$PREVIEW_SERVER <<'SSH_CMDS'
    //            set -e
    //            cd $TARGET_DIR
    //            docker load -i app-image.tar
    //            # Use a consistent container name (change as needed)
    //            docker rm -f ph-wh-admin || true
    //            docker run -d --name ph-wh-admin --restart unless-stopped -p 80:80 frontend:\${IMAGE_TAG} || true
    //          SSH_CMDS
    //        '''
    //     }
    //   }
